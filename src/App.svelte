<script>
  ...
  $: pack = d3.pack().size([width, innerHeight]).padding(3);
            ...
            <g transform="translate({pais.x}, {pais.y})">
              ...
              {#if currentStep < 5}
                ...
                <text
                  transition:fade
                  class="pais"
                  font-size={Math.min(14, pais.r / 3)}
                  text-anchor="middle"
        ...
        {#if currentStep >= 5}
          {#each root
            ...
            <g transform="translate({pais.x}, {pais.y})">
              ...
              />
              {#if pais.r > 32}
                <text class="pais" font-size={Math.min(14, pais.r / 3)}>
                  {pais.data.name}
                </text>
              {/if}
            </g>
          ...
          {#each root.descendants().filter((d) => d.depth === 1) as banco}
            <g transform="translate({banco.x}, {banco.y})">
              <text
                transition:fade
                class="banco"
                font-size={Math.min(14, banco.r / 2.5)}
                transform={currentStep > 2
                  ? `translate(0, ${banco.r * 0.8})`
                  : `translate(0, -2)`}
              ...
              {#if currentStep < 3}
                <text
                  transition:fade
                  class="activos"
                  font-size={Math.min(12, banco.r / 2.5)}
                  dy="1em"
                  text-anchor="middle"
                >
                  {banco.value.toLocaleString("es-ES", { useGrouping: true })}M€
                </text>
              {/if}
            </g>
          {/each}
